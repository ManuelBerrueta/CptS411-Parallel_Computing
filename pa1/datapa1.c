...
int main(int argc, char *argv[])
{
	...
	FILE * sendOutput = fopen("OutputSendBlocking.csv", "w");
	FILE * recvOutput = fopen("OutputRecvBlocking.csv", "w");
	...
	if (rank == 1)
	{
		int target = 1000; // num bytes to send for the current iteration
